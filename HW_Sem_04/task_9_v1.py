...
def loading_image(url: str, target_dir: str) -> None:
    ...
    with open(os.path.join(target_dir, filename), 'wb') as img:
        for data in response.iter_content(1024):
            img.write(data)
    print(f"Изображение загружено с <<{url}>> за {time.time() - start_thread_time:.2f} сек")
...
if __name__ == '__main__':
    ...
    threads = []
    for img_url in urls:
        thread = threading.Thread(target=loading_image, args=[img_url, IMAGE_DIR])
        threads.append(thread)
        thread.start()
    for thread in threads:
        thread.join()
    print(f'Общее время загрузки (многопоточный подход) => {time.time() - start_time:.2f} сек')
